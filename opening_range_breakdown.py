...
for symbol in symbols: 
    ...
    after_opening_bar_breakdown = after_opening_range_bars[after_opening_range_bars["close"] < opening_range_low]
    
    if not after_opening_bar_breakdown.empty: 
        if symbol not in existing_order_symbols: 
            limit_price = after_opening_bar_breakdown.iloc[0]["close"]
            print(f"Selling short {symbol} at {limit_price}, closed below {opening_range_low} at {after_opening_bar_breakdown.iloc[0]}")

            messages.append(f"Selling short {symbol} at {limit_price}, closed below {opening_range_low}\n\n{after_opening_bar_breakdown.iloc[0]}\n\n")
